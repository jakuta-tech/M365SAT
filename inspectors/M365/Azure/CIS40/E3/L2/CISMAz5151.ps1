...
function Build-CISMAz5151($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISMAz5151"
		FindingName	     = "CISMAz 5.1.5.1 - User consent to apps accessing company data on their behalf is allowed!"
...
function Audit-CISMAz5151
{
	try
	{
		...
		if (-not [string]::IsNullOrEmpty($UserConsentSetting) -and $UserConsentSetting -eq "ManagePermissionGrantsForSelf.microsoft-user-default-low")
		{
			...
			$finalobject = Build-CISMAz5151($UserConsentSetting)
			return $finalobject
		}
		return $null
	}
