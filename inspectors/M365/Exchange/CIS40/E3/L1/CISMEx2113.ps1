...
function Build-CISMEx2113($findings)
{
	...
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISMEx2113"
		FindingName	     = "CIS MEx 2.1.13 - An Connection filter safe list is on!"
		...
		PowerShellScript = 'Set-HostedConnectionFilterPolicy -Identity Default -EnableSafeList $false'
...
{	
	Try
	{
		...
		If ($HostedConnectionFilterPolicy.EnableSafeList -eq $true)
		{
			...
			$endobject = Build-CISMEx2113($HostedConnectionFilterPolicy)
			Return $endobject
		}
	}
	catch
