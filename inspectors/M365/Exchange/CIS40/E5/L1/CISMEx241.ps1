...
function Build-CISMEx241($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISMEx241"
		FindingName	     = "CIS MEx 2.4.1 - Priority account protection is not enabled and configured!"
...
function Audit-CISMEx241
{
	try
	{
		...
		if ($AffectedOptions.Count -ne 0)
		{
			...
			$finalobject = Build-CISMEx241($AffectedOptions)
			return $finalobject
		}
		return $null
	}
