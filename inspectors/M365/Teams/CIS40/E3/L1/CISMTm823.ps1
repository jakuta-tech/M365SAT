...
function Build-CISMTm823($findings)
{
	#Actual Inspector Object that will be returned. All object values are required to be filled in.
	$inspectorobject = New-Object PSObject -Property @{
		ID			     = "CISMTm823"
		FindingName	     = "CISM Tm 8.2.3 - External Teams users are allowed to initiate conversations"
		...
		PowerShellScript = 'Set-CsTenantFederationConfiguration -AllowTeamsConsumerInbound $false'
...
function Audit-CISMTm823
{
	try
	{
		...
		if ($ViolatedTeamsSettings.Count -igt 0)
		{
			...
			$endobject = Build-CISMTm823($ViolatedTeamsSettings)
			return $endobject
		}
		return $null
	}
